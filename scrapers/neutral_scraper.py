...
def _total_pages(soup, fallback=1):
    ...
    match = re.search(r"P[aá]gina\s*(\d+)\s*de\s*(\d+)", text)
    if match:
        try:
...
def _extract_products(soup, slug):
    ...
    for link in product_links:
        ...
        price_match = re.search(r"(?:USD|US\$)\s*\$?\s*([\d.,]+)", text)
        if not price_match:
            continue
        price = clean_price(text[price_match.start():price_match.end()])
        if price is None:
            continue

        name = text[price_match.end():].strip()
        if not name:
...
def run():
    ...
    save_products(unique, "neutral", out_dir)
    return unique
