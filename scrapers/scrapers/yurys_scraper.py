...
from utils import save_products, extract_wix_products, extract_wix_detail_price
...
def _load_previous():
    path = Path(__file__).parent.parent / "data" / "yurys.json"
    try:
...
async def _expand_wix_catalog(page, max_rounds=80):
    previous_count = -1
    stable_rounds = 0
    for _ in range(max_rounds):
        ...
        buttons = page.get_by_role(
            "button",
            name=re.compile(r"ver mais|mostrar mais|load more", re.I),
        )
    ...
    return await page.locator('[data-hook="product-item-root"]').count()
...
async def _scrape_category(context, slug):
    ...
    try:
        ...
        try:
            ...
            pass
        loaded = await _expand_wix_catalog(page)
        soup = BeautifulSoup(await page.content(), "html.parser")
        products = extract_wix_products(soup, "Yury's Free Shop", slug, BASE_URL)
        if not products:
            raise RuntimeError("no se encontraron tarjetas de producto")
        if loaded and len(products) < loaded:
            print(
                f"[Yury's] [aviso] {slug}: se cargaron {loaded} tarjetas "
                f"pero se leyeron {len(products)}"
            )
...
async def _run_async():
    ...
    save_products(merged, "yurys", Path(__file__).parent.parent / "data")
    return merged
