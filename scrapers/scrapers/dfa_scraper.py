...
def _load_previous():
    path = Path(__file__).parent.parent / "data" / "dfa.json"
    try:
...
def _total_pages(soup):
    ...
    calculated = math.ceil(total / PRODUCTS_PER_PAGE) if total else 1
...
def _merge_with_previous(fresh, previous):
    ...
    merged = {p.get("url") or p.get("nombre"): p for p in previous if isinstance(p, dict)}
...
def run():
    ...
    save_products(products, "dfa", Path(__file__).parent.parent / "data")
    return products
